...
avg_rev_per_loc_m   = fy25_revenue_m / total_locations
fcf_2025            = 2440.0
...
revenue_hist = pd.DataFrame({
    ...
    "StoreOpEx":       [14720.3, 15286.5, 17058.9],
})

trad_store_rev_k    = avg_rev_per_loc_m * 1e6 / 1e3
trad_store_opex_pct = 0.555
trad_labor_cost_k   = 400.0
trad_capex_k        = 750.0
...
with tab1:
    ...
    with col1:
        ...
        fig_gap.update_layout(
            ...
            template="simple_white", height=400, yaxis_title="Revenue ($B)",
...
| **+ 24/7 AI uplift ({uptime_mult:.1f}x)** | **${ai_rev_per_kiosk_k:,.0f}K** |
...
""")
        ...
        if surplus >= 0:
            ...
            st.warning(f"AI kiosks generate **${total_kiosk_rev_m/1000:.2f}B** — still **${abs(surplus)/1000:.2f}B** short")
...
""")
    with c2:
        ...
        st.markdown(f"""
- Revenue: **${ai_rev_per_kiosk_k:,.0f}K/yr** ({uptime_mult:.1f}x uplift)
...
- Personalization: **Real-time order prediction via Rewards data**
- Upsell capability: **AI suggests add-ons based on purchase history**
- Member recognition: **Displays your usual order before you tap**
""")
    with c3:
...
- Personalization uplift: **+10% to +40%** additional revenue potential
- Data flywheel: **Every transaction improves the model** for next visit
- Loyalty deepening: **Kiosk transactions feed Rewards tier progression**
""")

    # ── PERSONALIZATION SECTION ──────────────────────────────────
    st.divider()
    st.subheader("🎯 The Second AI Lever: Personalization")
    st.caption(
        "Beyond 24/7 operations, AI personalization through the Starbucks Rewards platform "
        "represents an additional revenue uplift not yet priced into the base model. "
        "Starbucks Rewards has 35.5 million active U.S. members whose order history, location patterns, "
        "and preferences are stored in the platform (Starbucks Corporation, 2026). "
        "McKinsey research shows personalization can increase revenue by up to 40% (McKinsey & Company, 2021)."
    )

    pers_uplift = st.slider("Personalization revenue uplift (%)", 0, 40, 10, 5,
                             help="Conservative estimate: 10%. McKinsey ceiling: 40%. Adjust to see impact.") / 100

    pers_rev_k     = ai_rev_per_kiosk_k * (1 + pers_uplift)
    pers_rev_m     = pers_rev_k / 1e3
    pers_kiosks    = int(round(growth_needed_m / pers_rev_m))
    pers_total_rev = pers_kiosks * pers_rev_k / 1e3
    kiosk_reduction = kiosks_needed - pers_kiosks

    p1, p2, p3, p4 = st.columns(4)
    p1.metric("Base Revenue per Kiosk",    f"${ai_rev_per_kiosk_k:,.0f}K",
              delta="24/7 ops only")
    p2.metric("With Personalization",      f"${pers_rev_k:,.0f}K",
              delta=f"+{pers_uplift:.0%} uplift")
    p3.metric("Kiosks Needed",             f"{pers_kiosks:,}",
              delta=f"{kiosk_reduction:,} fewer than base model", delta_color="normal")
    p4.metric("Total Revenue",             f"${pers_total_rev:.2f}B")

    pc1, pc2 = st.columns(2)
    with pc1:
        fig_pers = go.Figure()
        fig_pers.add_trace(go.Bar(
            x=["Base Model (24/7 only)", "With Personalization"],
            y=[ai_rev_per_kiosk_k, pers_rev_k],
            marker_color=["#1E3932", "#00704A"],
            text=[f"${ai_rev_per_kiosk_k:,.0f}K", f"${pers_rev_k:,.0f}K"],
            textposition="outside",
        ))
        fig_pers.update_layout(
            title="Revenue per Kiosk: Base vs Personalization-Enhanced ($K)",
            yaxis_title="Annual Revenue ($K)",
            template="simple_white", height=380, showlegend=False,
        )
        st.plotly_chart(fig_pers, use_container_width=True)

    with pc2:
        fig_kiosks = go.Figure()
        fig_kiosks.add_trace(go.Bar(
            x=["Base Model (24/7 only)", "With Personalization"],
            y=[kiosks_needed, pers_kiosks],
            marker_color=["#1E3932", "#00704A"],
            text=[f"{kiosks_needed:,}", f"{pers_kiosks:,}"],
            textposition="outside",
        ))
        fig_kiosks.update_layout(
            title="Kiosks Needed to Close the Growth Gap",
            yaxis_title="Number of Kiosks",
            template="simple_white", height=380, showlegend=False,
        )
        st.plotly_chart(fig_kiosks, use_container_width=True)

    # Personalization revenue range chart
    uplift_range   = [i/100 for i in range(0, 45, 5)]
    rev_range      = [ai_rev_per_kiosk_k * (1 + u) for u in uplift_range]
    kiosk_range    = [int(round(growth_needed_m / (r/1e3))) for r in rev_range]

    fig_range = go.Figure()
    fig_range.add_trace(go.Scatter(
        x=[f"+{u:.0%}" for u in uplift_range], y=kiosk_range,
        mode="lines+markers", line=dict(color="#00704A", width=3),
        text=[f"{k:,}" for k in kiosk_range], textposition="top center",
        name="Kiosks needed"
    ))
    if f"+{pers_uplift:.0%}" in [f"+{u:.0%}" for u in uplift_range]:
        fig_range.add_trace(go.Scatter(
            x=[f"+{pers_uplift:.0%}", f"+{pers_uplift:.0%}"],
            y=[0, max(kiosk_range)],
            mode="lines", line=dict(color="#CBA258", dash="dash", width=2),
            name="Current assumption", showlegend=True,
        ))
    fig_range.update_layout(
        title="Kiosks Needed vs Personalization Uplift Assumption",
        xaxis_title="Personalization Revenue Uplift",
        yaxis_title="Kiosks Needed",
        template="simple_white", height=360,
    )
    st.plotly_chart(fig_range, use_container_width=True)

    st.info(
        f"**How personalization works in the kiosk:** Each transaction is linked to a Rewards account, "
        f"enabling real-time order prediction, dynamic upsell suggestions based on past behavior, "
        f"and personalized promotions to drive repeat visits. "
        f"At a conservative {pers_uplift:.0%} personalization uplift, the number of kiosks needed "
        f"drops from **{kiosks_needed:,} to {pers_kiosks:,}** — closing the same growth gap "
        f"with {kiosk_reduction:,} fewer units deployed."
    )
    ...
    m4.metric("New Margin",          f"{combined_margin:.1%}",
              delta=f"{(combined_margin-current_margin)*100:+.1f}pp vs today", delta_color="normal")
    ...
    fig_margin.update_layout(
        title="Operating Margin: Before vs After AI Micro-Cafes",
...
with tab2:
    ...
    trad_roic = (trad_ebitda_k / trad_capex_k) * 100 if trad_capex_k > 0 else 0
    ai_roic   = (ai_ebitda_k   / ai_capex_k)   * 100 if ai_capex_k   > 0 else 0
    ...
    metrics_df = pd.DataFrame({
        ...
        "Traditional Store": [
            trad_store_rev_k, trad_opex_k, trad_labor_cost_k,
            trad_ebitda_k, trad_capex_k, round(trad_payback_yrs, 1), round(trad_roic, 0)
        ],
        "AI Micro-Cafe": [
            ai_rev_per_kiosk_k, ai_opex_k, ai_labor_k,
            ai_ebitda_k, ai_capex_k, round(ai_payback_yrs, 1), round(ai_roic, 0)
        ],
        "What This Means": [
            "Kiosk earns less per unit — expected given smaller size and menu",
            "Kiosk costs far less to run — lower rent, labor, overhead",
            "Biggest AI saving: automation replaces most shift labor",
            "Kiosk profit per unit is lower, but costs 80% less to build",
            "Key advantage: kiosk requires 80% less upfront investment",
            "Kiosk recoups its investment faster despite lower revenue",
            "Kiosk generates more profit per dollar invested — the real metric",
        ],
    })
    st.dataframe(
        metrics_df.style.format({"Traditional Store": "{:,.1f}", "AI Micro-Cafe": "{:,.1f}"}),
        use_container_width=True, hide_index=True
    )
    st.caption("Return on CapEx = EBITDA / CapEx. Kiosks generate more profit per dollar invested than traditional stores.")
    ...
    with c1:
        ...
        comp_fig.add_trace(go.Bar(name="AI Micro-Cafe",     x=cats, y=ai_vals,   marker_color="#00704A"))
...
with tab3:
    ...
    p3.metric("CapEx as % of FCF",     f"{pct_of_fcf:.0f}%",
              delta="Manageable" if pct_of_fcf <= 100 else "Needs phasing",
              delta_color="normal" if pct_of_fcf <= 100 else "inverse")
...
with tab5:
    ...
    for profile, vals in profiles.items():
        ...
        for i, yr in enumerate(yrs_labels):
            rows.append({
                ...
                "Cumulative EBITDA ($M)":  round(cum[i] * ai_ebitda_k / 1e3, 1),
    ...
    st.dataframe(
        sel[["Year", "New Kiosks", "Cumulative Kiosks",
             "Cumulative Revenue ($M)", "Cumulative EBITDA ($M)", "Annual CapEx ($M)"]],
        use_container_width=True, hide_index=True
    )
    ...
    for profile in profiles:
        ...
        payback_yr = next(
            (s["Year"].iloc[i] for i, (c, e) in enumerate(zip(cum_capex, cum_ebitda)) if e >= c),
            "Beyond ramp"
        )
